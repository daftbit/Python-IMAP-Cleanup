...
if __name__ == '__main__':
	...
	# test if directory for logs exists
	if not os.path.isdir(os.path.join(os.getcwd(), 'logs')):
		# create it if it doesn't
		os.mkdir('logs')
	logging.basicConfig(filename=os.path.join(os.getcwd(),'logs/') + str(date_time.strftime("%Y%m%d-%H%M%S-")) + 'imap_data.log',level=logging.DEBUG)
